...
FROM
	data_analyst_jobs
WHERE
	review_count >= 500 and review_count <= 1000;
...
SELECT
	location AS state, ROUND(AVG(star_rating), 2) AS avg_rating
FROM
	data_analyst_jobs
WHERE
	location IS NOT NULL AND star_rating IS NOT NULL
GROUP BY
	location
ORDER BY 
	avg_rating DESC
...
SELECT
	company, AVG(star_rating) AS avg_star_rating
FROM
	data_analyst_jobs
WHERE
...
FROM
	data_analyst_jobs
WHERE
	title NOT ILIKE '%analyst%' AND title NOT ILIKE '%analytics%';
...
SELECT
	domain, COUNT(title) AS hard_to_fill
FROM
	data_analyst_jobs
WHERE
	skill ILIKE '%sql%' AND days_since_posting > 21 AND domain IS NOT NULL 
GROUP BY
	domain
ORDER BY
	hard_to_fill DESC;